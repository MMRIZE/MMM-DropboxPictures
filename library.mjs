...
class ImageList {
  ...
  loadScanned(scanned) {
    ...
    const validSize = (size) => {
      ...
      //if (!res) log((`[DBXP] ${size} is out of range. (fileSizeMinMax:${fileSizeMinMax})`)
      return res
    }
    const validTime = (time, range) => {
      ...
      //if (!res) log(`[DBXP] ${t} is out of range. (timeMinMax:${range})`)
      return res
    }
    ...
    if (this.#images.length > this.#options.maxImages) {
      log(`[DBXP] Too many images will be cut. (maxImages:${this.#options.maxImages}, images:${this.#images.length})`)
      this.#images.length = this.#options.maxImages
    }
    if (this.#images.length <= 0) {
      log('[DBXP] No image to serve.')
    }
    this.#index = 0
  }
